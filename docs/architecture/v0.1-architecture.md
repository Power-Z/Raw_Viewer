...
- LibRaw 未支持的相机厂商 RAW，或对所有厂商格式作无条件兼容承诺；
...
- `FlatBinaryRawDecoder`：按 `RawDescriptor` 验证并解释平面 `.raw/.RAW/.bin`；
- `CameraRawDecoder`：通过主线 LibRaw 读取 TIFF/DNG/厂商相机 RAW 容器及未处理 Bayer 数据；
- `ImageFormatProbe`：先检查内容签名，再参考扩展名选择解码器；
...
第一阶段的平面 RAW 只支持 byte-aligned 8/16/32-bit；packed 10/12/14-bit 使用独立解码器在后续加入。

### 7.4 相机 RAW 容器

文件扩展名不能决定数据布局。已登记样本 `B0012535.raw` 以 `49 49 2A 00` 开头，是 little-endian TIFF 容器，包含 IFD、缩略图、相机元数据和 RAW 数据。对这类文件：

- 禁止将 `fileSize - width * height * bytesPerSample` 当作 `headerBytes`；
- 使用 `ImageFormatProbe` 识别 TIFF/DNG/厂商签名；
- 使用 LibRaw 获取相机型号、RAW/活动区尺寸、边距、Bayer pattern、黑白电平、方向和未处理像素；
- 容器中的物理 strip/tile/压缩布局由 LibRaw 处理，不暴露为平面 RAW 的 `rowStride`；
- 解码后的未处理数组仍进入同一个不可变 `OriginalData` 和瓦片接口；
- 元数据探测与完整像素解码分离，文件列表预览不应先解码整幅 RAW；
- LibRaw 不支持、文件损坏或多图像语义不明时返回结构化错误，不回退到盲目的平面解释。

当前样本经 LibRaw 0.22.0 验证：

| 项目 | 结果 |
|---|---|
| 相机 | Hasselblad X2D 100C |
| 文件大小 | 211,714,048 bytes（201.906 MiB） |
| RAW 数组 | 11904×8842 UInt16，decoded stride 23,808 bytes |
| 活动区 | 11664×8750，left=124，top=92 |
| Bayer | RGGB |
| 黑电平 | 4094 / 4093 / 4093 / 4094 |
| 白电平 | 65535 |
| 缩略图 | 3888×2918 |

用户提供的 `11776×8842` 与容器元数据不一致，不能作为该文件的平面解码参数。
...
- 平面 RAW 使用只读内存映射或分块读取；相机 RAW 的压缩/容器访问先经过 LibRaw；
...
- 第三方依赖锁定版本和来源，记录许可证；
- 相机 RAW 使用主线 LibRaw，不启用 GPL demosaic packs；当前官方稳定基线为 0.22.x，升级需运行相机样本回归；
...
| ID | 问题 | 结论 | 状态 |
|---|---|---|---|
| D-001 | 首发平台 | Windows 10/11 x64 | 已确认 |
| D-002 | Qt 许可 | 开发期按 LGPL 动态链接方案；正式发布前再次审查 | 临时确认，发布门槛 |
| D-003 | 32-bit RAW 类型 | 同时支持 UInt32 / Float32 | 已确认 |
| D-004 | Bayer pattern 所属 | 作为每个 RAW 文档参数；当前样本为 RGGB | 已确认 |
| D-005 | row stride | 平面 RAW 为高级参数；相机容器由 LibRaw 管理 | 已确认 |
| D-006 | BLV 语义 | 拆分 sensorBlackLevel 与 displayBlackPoint | 架构默认，待 UI 验收 |
| D-007 | Horizontal/Vertical | 采用第 11 节定义 | 已确认 |
| D-008 | 性能设备 | Windows 11 x64、Intel Core Ultra 5、16 GiB 显存；其余规格待补 | 已确认基线 |
| D-009 | 普通压缩 200 MP 图片 | 初版尽力支持并警告；性能承诺先限定 RAW | 风险仍开放 |
| D-010 | 200 MP / 400 MB 样本 | 延期到 V0.4 准备；不阻塞 V0.2 | 已批准延期 |
| D-011 | 相机 RAW 容器 | 使用内容探测 + 主线 LibRaw，与平面解码器分离 | 已确认 |
...
- LibRaw 官方文档：<https://www.libraw.org/docs>
- LibRaw 相机支持：<https://www.libraw.org/supported-cameras>
- LibRaw 许可：<https://www.libraw.org/about>
